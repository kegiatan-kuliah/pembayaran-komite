...
@section('content')
    <div class="row">
        <div class="col-lg-6">
            {{ html()->form('POST', route('siswa.update'))->open() }}
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        {{ html()->input('text', 'nama', $data->user->nama)
                        ->class('form-control')->attribute('required', true)
                        ->attribute('placeholder', 'Isikan nama') }}
                    </div>
                    <div class="mb-3">
                        {{ html()->label('NISN', 'nisn')->class('form-label') }}
                        {{ html()->input('text', 'nisn', $data->nisn)
                        ->class('form-control')->attribute('required', true)
                        ->attribute('placeholder', 'Isikan nisn') }}
                    </div>
                    <div class="mb-3">
                        {{ html()->label('Kelas', 'id_kelas')->class('form-label') }}
                        {{ html()->select('id_kelas', ['' => 'Pilih Kelas'] + $kelas->toArray(), $data->id_kelas)->attribute('required', true)
                                ->class('form-control')
                        }}
                    </div>
                    <div class="mb-3">
                        {{ html()->label('Angkatan', 'id_angkatan')->class('form-label') }}
                        {{ html()->select('id_angkatan', ['' => 'Pilih Angkatan'] + $angkatans->toArray(), $data->id_angkatan)->attribute('required', true)
                                ->class('form-control')
                        }}
                    </div>
                    <div class="mb-3">
                        {{ html()->label('Alamat', 'alamat')->class('form-label') }}
                        {{ html()->textarea('alamat', $data->alamat)
                        ->class('form-control')->attribute('required', true)
